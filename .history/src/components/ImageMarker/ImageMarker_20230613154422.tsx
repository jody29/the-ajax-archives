...
export function ImageMarker(props: ImageMarkerProps) {
  ...
  return (
    <Link href={`/verhalen/${props.link}`} passHref>
      <Flex as="a" alignItems='center' flexDir='column' transform='translateY(-33px)'  onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        <Box bg='white' py={1} px={2} minW='100px' boxShadow='0 0 0 1px black' mb={4} borderRadius='4px' transform={isHovered ? 'scale(1)' : 'scale(0)'} opacity={isHovered ? 1 : 0} transition='transform 0.4s ease-in-out, opacity 0.4s ease-in-out' >
          <Text color='red' fontWeight='bold'>Club(s)</Text>
          {props.clubs.map(club => (
            <Text key={club}>{club}</Text>
          ))}
        </Box>
