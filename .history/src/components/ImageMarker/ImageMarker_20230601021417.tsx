...
export function ImageMarker(props: ImageMarkerProps) {
  ...
  return (
    <Link href={`/verhalen/${props.link}`} passHref>
      <Flex as="a" alignItems='center' flexDir='column' position='relative' onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        <Box bg='white' p={1} position={isHovered ? 'relative' : 'absolute'} minW='100px' boxShadow='0 0 0 1px black' mb={4} borderRadius='4px' transform={isHovered ? 'scale(1)' : 'scale(0)'} opacity={isHovered ? 1 : 0} transition='transform 0.4s ease-in-out, opacity 0.4s ease-in-out' >
