          ...
          <Marker key={story.sys.id} latitude={story.fields.locatie.lat} longitude={story.fields.locatie.lon} >
            <Link href={`/verhalen/${story.sys.id}`} passHref>
              <Flex as="a" alignItems='center' flexDir='column'>
                <Image src={story.fields.thumbnail?.fields.file.url} borderRadius='50%' w={14} h={14} objectFit='cover' mb={2} />
                <Text color='red' fontWeight='bold' fontSize='1rem'>{story.fields.plaatsnaam}</Text>
              </Flex>
            </Link>
          </Marker>
        ))}
