        ...
        {props.verhalen.map(story => (
          <Marker key={story.sys.id} latitude={story.fields.locatie.lat} longitude={story.fields.locatie.lon} >
            <Link href={`/verhalen/${story.sys.id}`} passHref>
              <Flex as="a" alignItems='center' flexDir='column' position='relative'>
                <Box bg='white' p={2} minW='100px' boxShadow='0 0 0 1px black' position='absolute' top='-50%' >
                  <Text color='red'>Club(s)</Text>
                </Box>
