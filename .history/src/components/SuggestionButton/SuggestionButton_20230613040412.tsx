...
export const SuggestionButton = (props: SuggestionButtonProps) => {
  ...
  return (
    <Button variant='secondary' display='flex' alignItems='center' padding={0} color='black' w='fit-content' fontSize='1.2rem' fontWeight='bold' position='relative' onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeavle}>
      <Text bg='white' zIndex={10}>{props.suggestion}</Text>
      <Icon right={-10} transform={`translateY(11%) ${isHovered ? 'translateX(0)' : 'translateX(-300%)'}`} opacity={isHovered ? 1 : 0} transition='transform 0.4s ease-in-out, opacity 0.4s ease-in-out' fontSize='1.3rem' position='absolute'>
        <FaChevronRight />
      </Icon>
    </Button>
  );
