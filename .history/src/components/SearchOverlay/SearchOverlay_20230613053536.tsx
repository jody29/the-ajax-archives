  ...
  useEffect(() => {
    const fetchData = async () => {
      if (inputValue.length > 0) {
        const collectie = await axios.get(`https://cdn.contentful.com/spaces/${process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID}/environments/master/content_types/ticketAjaxAcMilan1995?acces_token=${process.env.NEXT_PUBLIC_CONTENTFUL_ACCESS_TOKEN}`)
        const verhalen = await axios.get(`https://cdn.contentful.com/spaces/${process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID}/environments/master/content_types/stories?acces_token=${process.env.NEXT_PUBLIC_CONTENTFUL_ACCESS_TOKEN}`)
