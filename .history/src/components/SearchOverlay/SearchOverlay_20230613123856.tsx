  ...
  useEffect(() => {
    const fetchData = async () => {
      if (inputValue.length > 0) {
        const collectie = await axios.get(`https://cdn.contentful.com/spaces/${process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID}/environments/master`, {
          params: {
            acces_token: process.env.NEXT_PUBLIC_CONTENTFUL_ACCESS_TOKEN,
            content_type: 'ticketAjaxAcMilan1995'
          }
        })
        const verhalen = await axios.get(`https://cdn.contentful.com/spaces/${process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID}/environments/master`, {
          params: {
            acces_token: process.env.NEXT_PUBLIC_CONTENTFUL_ACCESS_TOKEN,
