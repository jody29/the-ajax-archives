...
const options = {
    renderNode: {
        [BLOCKS.EMBEDDED_ASSET]: (node: EmbeddeAssetProps) => {
            const { title, file } = node.data.fields
...
const Page: NextPage<PageProps> = props => {
  ...
  console.log(props.story.fields.verhaal?.content[4])

  const formatText = (text: string) => {
    return text.replace(/__(.*?)__/g, "<strong>$1</strong>").replace(/\n/g, "<br>")
  }

  const renderedStory = documentToReactComponents(props.story.fields.verhaal)
  ...
  return (
    <>
      ...
      <Box>{documentToReactComponents(props.story.fields.verhaal)}</Box>
      <Footer />
    </>
  );
};
